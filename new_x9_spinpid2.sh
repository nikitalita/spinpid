...
function read_fan_data {
   ...
   # the script will assume the duty cycles are what was last set.
	if [ $HOW_DUTY == 1 ] ; then
		DUTY_CPU=$($IPMITOOL raw $DUTYCMD 0 $ZONE_CPU) # in hex with leading space
		DUTY_CPU=$((0x$(echo $DUTY_CPU)))  # strip leading space and decimalize
		DUTY_PER=$($IPMITOOL raw $DUTYCMD 0 $ZONE_PER)
...
function adjust_fans {
   ...
   if [[ $DUTY -ne $DUTY_LAST ]] || [[ FIRST_TIME -eq 1 ]]; then
      ...
      echo -n "$($IPMITOOL raw $DUTYCMD 1 "$ZONE" "$DUTY")"
   fi
   FIRST_TIME=0
}
...
if [[ ${!RPM_PER} -ge RPM_PER_MAX || -z ${DUTY_PER+x} ]]; then
   echo -n "$($IPMITOOL raw $DUTYCMD 1 $ZONE_PER 50)"
   DUTY_PER=50; sleep 1
fi
if [[ ${!RPM_CPU} -ge RPM_CPU_MAX || -z ${DUTY_CPU+x} ]]; then
   echo -n "$($IPMITOOL raw $DUTYCMD 1 $ZONE_CPU 50)"
   DUTY_CPU=50; sleep 1
fi
