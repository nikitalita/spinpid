...
LOG=/var/log/fancontrol/fan.log  # Change to your desired log location/name
exec > >(tee -i $LOG) 2>&1

# CPU output sent to a separate log for interim cycles
CPU_LOG=/var/log/fancontrol/cpu.log
...
RPM_CPU_30=4000   # adapt these for your system
RPM_CPU_MAX=7000
RPM_PER_30=3450
RPM_PER_MAX=7125
...
DRIVE_T=1  # time interval for checking drives (minutes).  Drives change
...
IPMITOOL="/usr/bin/ipmitool -I lanplus -H 192.168.1.80 -U fancontrol -Pfancontrol"
...
function print_header {
   ...
   printf "\n%-*s %3s %16s %29s \n" $SPACES "$DATE" "CPU" "New_Fan%" "New_RPM___________________________________"
   echo -n "          "
   while read -r LINE ; do
      get_disk_name
      printf "%-5s" "$DEVID"
   done <<< "$DEVLIST"             # while statement works on DEVLIST
   printf "%4s %5s %6s %6s %5s %6s %3s %-7s %s %-4s %5s %5s %5s %5s %5s %5s %5s %5s" "Tmax" "Tmean" "ERRc" "P" "I" "D" "TEMP" "MODE" "CPU" "PER" "FANA" "FANB" "FAN1" "FAN2" "FAN3" "FAN4" "FAN5" "FAN6"
}
...
function CPU_check_adjust {
   ...
   if [[ $CPU_TEMP1 -gt $CPU_TEMP2 ]]; then CPU_TEMP=$CPU_TEMP1; else CPU_TEMP=$CPU_TEMP2; fi


   #CPU_TEMP=$CPU_TEMP2
   #CPU_TEMP="$(( CPU_TEMP1 > CPU_TEMP2 ? CPU_TEMP1 : CPU_TEMP2 ))"
   ...
   if [[ $DUTY_CPU -gt $DUTY_PER ]]; then
      adjust_fans $ZONE_PER $DUTY_CPU $DUTY_CPU_LAST
   else
...
echo "Number of drives :  $DEVCOUNT"
...
while true ; do
   ...
   printf "%7s %6s %5s %6.6s %4s %-7s %3d %3d %6s %5s %5s %5s %5s %5s %5s %5s" "${ERRc:----}" "${P:----}" $I "${D:----}" "$CPU_TEMP" $MODEt $DUTY_CPU $DUTY_PER "${FANA:----}" "${FANB:----}" "${FAN1:----}" "${FAN2:----}" "${FAN3:----}" "${FAN4:----}" "${FAN5:----}" "${FAN6:----}"
